...
| **Subnets (auto iSCSI)**           | GET `subnets`, GET `subnets/:id`                                                                   | List may be summary-only (`id`, `name`); plugin hydrates missing `discovery_ip` via GET `subnets/:id`. Discovery portals: subnets whose `type` contains `data` and have `discovery_ip`; then loose pass (any `discovery_ip`); `network_interfaces`; `iscsi_discovery_ips`; `iscsiadm -m session`. |
...
- **Paths:** tokens, initiator_groups, initiators (not used; initiator_groups accepts iscsi_initiators inline), volumes, volumes/:id, volumes/:id/actions/restore, access_control_records, snapshots, snapshots/:id, pools, volume_collections, subnets, subnets/:id.
