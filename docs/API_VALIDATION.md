...
| **Subnets (auto iSCSI)**           | GET `subnets`, GET `subnets/:id`, GET `network_interfaces/:id`                                     | Subnet list may be summary-only; hydrate `discovery_ip` via GET `subnets/:id`. `network_interfaces` list may omit `ip_list`; hydrate via GET `network_interfaces/:id`. Portal order: **IPs from live `iscsiadm` sessions first**, then subnets (type `data` preferred), nif, `iscsi_discovery_ips`. Serial match: normalized / substring for API vs sysfs. |
