...
| **Subnets (auto iSCSI)**           | GET `subnets`                                                                                      | Read; discovery_ip, allow_iscsi, type                                                                                                                                                                        | OK; `nimble_data_as_list` unwraps `data.items[]`; optional `iscsi_discovery_ips`; if still empty, portal IPs from `iscsiadm -m session` (host fallback).                |
