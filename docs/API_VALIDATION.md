...
| PUT `volumes/:id`                  | size (resize), name (rename), **online** (helpers **`nimble_volume_ensure_offline`** / **`nimble_volume_ensure_online`** for restore and delete prep) | Update                                                                                                                                                                                                       | OK                                                                                                       |
...
| DELETE `access_control_records/:id` | Remove ACR before volume delete (migration rollback / free_image)                               | Delete                                                                                                                                                                                                       | OK                                                                                                       |
| **Snapshots**                      |                                                                                                    |                                                                                                                                                                                                              |                                                                                                          |
...
- **Array requirement:** Restore fails with **409** / **`SM_vol_not_offline_on_restore`** if the volume is still **online** on the Nimble array. Host-only deactivate (unmap + remove ACR) does not set **`online=false`**.
- **Plugin sequence:** (1) **`nimble_volume_ensure_offline`** — GET **`volumes/:id`**; PUT **`{ online: false }`** only when the volume is online; if PUT fails, GET again and **die** unless the volume is now offline (handles redundant PUT when already offline). (2) POST **`volumes/:id/actions/restore`** with **`id`** + **`base_snap_id`**. (3) **`nimble_volume_ensure_online`** — PUT **`{ online: true }`** up to **3** attempts (**2** s apart); after each successful PUT, GET **`volumes/:id`** and require **`online`** true before succeeding; **die** if still failing (PVE **`activate_volume`** does not set **`online`** on the array). (4) If restore failed, **`nimble_volume_ensure_online`** is still attempted; the primary error is still the restore failure; if bring-online also failed, a **warning** logs the secondary error before **`die`** with the restore error.
- **Shared helpers:** **`nimble_volume_detail`** (GET **`volumes/:id`**, returns **`data`** hash or undef), **`nimble_volume_ensure_offline`**, **`nimble_volume_ensure_online`**. **`nimble_offline_volume_and_delete_snapshots`** (volume delete path) takes an optional **PVE volume name** label for errors and uses **`nimble_volume_ensure_offline`** before deleting snapshots, same offline semantics as rollback.
