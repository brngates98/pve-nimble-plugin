...
| **Access control**                 |                                                                                                    |                                                                                                                                                                                                              |                                                                                                          |
| GET `access_control_records`       | List all; client-side filter uses **`nimble_acr_vol_id`** (`vol_id` or **`volume_id`** on row) plus **`initiator_group_id`** (disconnect, delete-for-volume, fallback in **`nimble_volume_has_acl_for_ig`**) | Read                                                                                                                                                                                                         | OK; list rows may use **`volume_id`** instead of **`vol_id`** on some firmware.                          |
...
| GET `snapshots`                    | (1) `?vol_id=` (URI-escaped): delete, rollback helpers, `volume_snapshot_info`, `nimble_delete_snapshots_for_volume_id` (optional unfiltered fallback if filtered empty). (2) **`nimble_sync_array_snapshots`**: tries unfiltered GET first; on failure (e.g. **400** / **`SM_missing_arg`** — filter required on some firmware), merges **`GET snapshots?vol_id=`** per PVE volume. On some firmware, filtered rows omit **`vol_name`** / **`vol_id`** and **`creation_time`**; the plugin sets **`vol_name`** from the volume context (or **`vol_id`→volume** map on bulk lists) and derives a sort/sync time via **`nimble_snapshot_effective_creation_time`** (`creation_time`, **`last_modified`**, **`NSs-…-YYYY-MM-DD::HH:MM:SS`** in **`name`**, else stable hash of **`id`**). | Read                                                                                                                                                                                                         | **Verify on array**: unfiltered list may be rejected; plugin handles per-volume merge. Fallback unfiltered in delete path may 400 on same firmware (filtered path is primary). Sparse snapshot fields observed on some arrays — plugin compensates. |
| GET `snapshots/:id`, `snapshots?id=`, **`volumes/:id`**, **`snapshot_collections/:id`** | When a list row has **`id`** but no plausible time for PVE **snaptime**, **`nimble_hydrate_snapshot_detail`** merges, in order: snapshot by id, **`GET volumes/:id`** (HPE model: snapshot is a volume), then **`snap_collection_id`** → **`snapshot_collections`** for collection **`creation_time`**. **`volume_snapshot_info`** uses the same helper (collection cache per call). | Read                                                                                                                                                                                                         | **Verify on array**: path vs **`id=`** query support may vary; failures are non-fatal (eval). |
...
| GET `access_control_records` (deactivate path) | `nimble_volume_connection` disconnect: fetches ALL ACRs (no `vol_id` filter), then filters client-side by **`nimble_acr_vol_id`** + `ig_id` | Read — `vol_id` query param not verified | **Improvement**: filtered `access_control_records?vol_id=<id>` to reduce payload on large arrays. Not a correctness issue. |
...
| **Array snapshot sync**            | `nimble_sync_array_snapshots`: GET `volumes` (vol→vmid map), then GET `snapshots` unfiltered when allowed, else **`GET snapshots?vol_id=`** per volume and merge client-side; matches snapshots to volumes using effective **`vol_name`**. PVE snapshot **name** stays **`nimble` + identity** (real epoch or stable **`id`** hash when the array omits time); **snaptime** in the VM config uses **`nimble_snapshot_display_epoch`** (plausible API times, **`NSs-…`** in **`name`**, else import-time fallback) so the UI does not show 1970s dates. Delete/rollback resolve imported **`nimble`** numeric suffix via **`nimble_snapshot_time_distance_for_nimble_suffix`** (±60s for real epochs, exact match for synthetic/hash or raw sub-plausible API integers). Throttled in `status()` (~30s). | GET volumes, GET snapshots                                                                                                                                                                                    | OK; cross-volume groups use merged per-volume snapshot lists when bulk read is not supported. If the array omits real **`creation_time`** on all disks, multi-disk “same instant” groups may not align (single-disk VMs still sync). |
...
| `snapshot_collections`, `snapshot_collections/:id` | `snapshot_collections.py` | Optional read when hydrating snapshot **snaptime** (collection **`creation_time`**). |
| `pools` | `pools.py` | |
...
- **`nimble_delete_snapshots_for_volume_id`:** Up to **5** rounds, newest-first by **`nimble_snapshot_effective_creation_time`**, **1** s between rounds; warns on failed snapshot **DELETE** (silent failures previously left snapshots and caused **`SM_eperm`** on volume **DELETE**). Rows without **`vol_id`** are still deleted when the list came from **`GET snapshots?vol_id=`** (same as rollback/delete matching).
