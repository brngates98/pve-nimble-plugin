...
**Automated helper (read-only by default):** `./scripts/nimble_api_unknowns_probe.sh` — prompts for API URL, username, and password (same as **`nimble_capacity_api_probe.sh`**), runs the checks above, and **writes one structured JSON object to a file** (default `nimble-unknowns-probe-<UTC-timestamp>.json` in the current directory; optional path argument overrides). Prints `Wrote <path>` on stderr. Login failures emit JSON on stderr and exit non-zero. Optional env: **`NIMBLE_VOL_ID`**, **`VERIFY_SSL=1`**, **`RUN_INLINE_IG_PROBE=1`**, **`RUN_MULTI_INITIATOR_PUT=1`**. See script header for details.
