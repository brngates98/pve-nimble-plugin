...
| GET `snapshots`                    | (1) `?vol_id=` (URI-escaped): delete, rollback helpers, `volume_snapshot_info`, `nimble_delete_snapshots_for_volume_id` (optional unfiltered fallback if filtered empty). (2) **`nimble_sync_array_snapshots`**: tries unfiltered GET first; on failure (e.g. **400** / **`SM_missing_arg`** — filter required on some firmware), merges **`GET snapshots?vol_id=`** per PVE volume. | Read                                                                                                                                                                                                         | **Verify on array**: unfiltered list may be rejected; plugin handles per-volume merge. Fallback unfiltered in delete path may 400 on same firmware (filtered path is primary). |
...
| **Array snapshot sync**            | `nimble_sync_array_snapshots`: GET `volumes` (vol→vmid map), then GET `snapshots` unfiltered when allowed, else **`GET snapshots?vol_id=`** per volume and merge client-side; writes PVE QEMU configs. Throttled in `status()` (~30s). | GET volumes, GET snapshots                                                                                                                                                                                    | OK; cross-volume groups use merged per-volume snapshot lists when bulk read is not supported. |
