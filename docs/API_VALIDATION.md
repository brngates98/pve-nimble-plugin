...
| GET `snapshots/:id`, GET `snapshots?id=` | When a list row has **`id`** but no plausible **`creation_time`** (or related) for PVE **snaptime**, **`nimble_hydrate_snapshot_detail`** merges **GET snapshots/:id** (fallback **GET snapshots?id=**) into the row (deduped per id during sync). Also used from **`volume_snapshot_info`**. | Read                                                                                                                                                                                                         | **Verify on array**: path vs **`id=`** query support may vary by firmware; failures are non-fatal (eval). |
