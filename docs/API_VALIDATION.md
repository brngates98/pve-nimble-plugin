...
| **Array snapshot sync**            | `nimble_sync_array_snapshots`: GET `volumes` (vol→vmid map), then GET `snapshots` unfiltered when allowed, else **`GET snapshots?vol_id=`** per volume and merge client-side; matches snapshots to volumes using effective **`vol_name`**; groups use effective creation time. Writes PVE QEMU configs. Throttled in `status()` (~30s). | GET volumes, GET snapshots                                                                                                                                                                                    | OK; cross-volume groups use merged per-volume snapshot lists when bulk read is not supported. If the array omits real **`creation_time`** on all disks, multi-disk “same instant” groups may not align (single-disk VMs still sync). |
