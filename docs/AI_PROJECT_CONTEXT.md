...
| **Array snapshot sync** | Implemented | `nimble_sync_array_snapshots` runs from `status()` (throttled to once per 30s per storage). Uses **`GET snapshots`** when the array allows an unfiltered list; otherwise merges **`GET snapshots?vol_id=`** per PVE volume (some firmware returns **400** / **`SM_missing_arg`** without a filter). Imports array-created snapshots into PVE VM configs; array snaps get `nimble<epoch>` keys. **QEMU VMs only;** CT roots skipped (no `LXCConfig` sync). |
