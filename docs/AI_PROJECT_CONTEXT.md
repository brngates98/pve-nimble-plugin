...
- **Config (storage.cfg):** `address`, `username` (legacy `nimble_user` is merged into `username` in `check_config`), optional `initiator_group` (if unset, plugin auto-creates/uses `pve-<nodename>` with local IQN), optional `vnprefix`, `pool_name`, optional **`volume_collection`**, `check_ssl`, `token_ttl`, `debug`, optional **`auto_iscsi_discovery`**. **`password` is sensitive** (ESXi-style): `pvesm` / API pass it via `on_add_hook`; the plugin writes **`/etc/pve/priv/nimble/<storeid>.pw`**. Plaintext `password` in `storage.cfg` still works for hand-edited configs; API login uses `password` / `nimble_password` from `$scfg` if set, otherwise the `.pw` file.
- **Nimble API base:** `https://<address>:5392/v1/`. Auth: POST `tokens` with username/password → use `session_token` as `X-Auth-Token` on later requests. List responses may use `data: { items: [ ... ] }`; `nimble_data_as_list` unwraps `items`. Discovery portals: GET `subnets`, optional **`iscsi_discovery_ips`**, else **`iscsiadm -m session`** TCP portal parse (host already logged into Nimble). Pure plugin differs: API **connections** + no `iscsiadm` in `map_volume` ([pve-purestorage-plugin](https://github.com/kolesa-team/pve-purestorage-plugin)).
...
- **PVE property registry:** Do **not** declare `username` / `password` in Nimble’s **`properties()`** — RBD/CIFS already register them globally. List **`username` + `password` only in `options()`** (like **ESXi**). **`password`** must be listed in **`plugindata → sensitive-properties`**; it is **not** present in `$config` during `check_config` — **`on_add_hook`** receives it and writes **`/etc/pve/priv/nimble/<storeid>.pw`**. **`nimble_api_credentials`** reads `password` / `nimble_password` from `$scfg` if present, else the `.pw` file. **`pvesm`** uses underscores (`--auto_iscsi_discovery`), not hyphens, for multi-word options.
