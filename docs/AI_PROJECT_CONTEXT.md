...
- **Worker password:** Some PVE tasks pass a redacted `$scfg`; **`nimble_api_credentials`** also tries **`/etc/pve/priv/nimble/<id>.pw`** for **`storage`** / **`storagename`** when **`$storeid`** is empty or the file is missing under the primary id.
