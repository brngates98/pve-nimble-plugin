...
| **Disk grow (resize)** | Implemented | **`volume_resize`** → **`nimble_resize_volume`**: **`PUT volumes/:id`** with larger **`size`** (MB), then **`nimble_host_refresh_volume_size_after_array_resize`** so Linux sees the new LUN immediately — **`iscsiadm -m session --rescan`**, **`multipath -v2`**, **`scsi_scan_new('iscsi')`**, per-slave **`sd*`** sysfs rescan, **`multipathd resize map`** (friendly alias + WWID variants), **`udevadm settle`**, then **`wait_for`** until **`blockdev --getsize64`** ≥ requested bytes (timeout **90s**, **0.5s** delay; retries **`multipathd resize`** every ~**10s** while polling). Matches standard online iSCSI/multipath grow; avoids “array shows new size, Proxmox task fails / Hardware shows old size until stop-remap.” If no block device exists on the task node, host refresh runs but **`wait_for`** is skipped (shared-storage edge case). |
...
| **Unit tests** | Present | test_command_validation.t, test_retry_logic.t, test_token_cache.t, test_nimble_plugin_import_export_guards.t, test_storage_api_ver14.t (+ token_cache_test.pl); no live Nimble tests |
...
- **Disk grow:** After the Nimble API accepts a larger **`size`**, the plugin refreshes iSCSI sessions, SCSI paths, and multipath maps on the node running the resize task so **`blockdev`** matches the array before PVE updates the VM config or QEMU. **`block_device_action`** rescans **`sd[a-z]+`** slaves only (Nimble presents iSCSI as **`sd*`**; not used for NVMe/vd).
...
- **`sub api`:** Use `PVE::Storage::APIVER()` and `PVE::Storage::APIAGE()` with parentheses and `use PVE::Storage ();` — they are `use constant` subs, not `$PVE::Storage::APIVER` (wrong — uninitialized). Bareword `PVE::Storage::APIVER` without `()` fails **strict subs** on Perl 5.36+ (e.g. CI bookworm `perl -c`). **`$tested_apiver = 14`**: on PVE **APIVER 13** the plugin still returns **13**; on **APIVER 14+** it returns **14** (silences “older storage API” warning). Minimum supported via **APIAGE** is **9** (PVE enforces **`api() >= APIVER - APIAGE`**).
...
*Last updated: **Disk resize** — host-side iSCSI/multipath refresh after **`PUT volumes/:id`** **`size`** (**`nimble_host_refresh_volume_size_after_array_resize`**). Prior v0.0.21 notes: VM Disks **Date** (**`ctime`**); migration **unmap** multipath teardown non-fatal; snapshot sync skips duplicate **`.snap-*`** rows; optional **deploy-nimble-plugin-pm.sh**. Update this file when you make significant changes or when status/next steps change.*
