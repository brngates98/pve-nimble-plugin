...
- **Worker password / import child:** **`nimble_api_credentials`** and **`nimble_api_call`** (token cache path) use **`nimble_effective_storeid($scfg, $storeid)`** so login works when PVE omits `$storeid` but sets **`storage` / `storagename` / `storeid` / `name` / `id` / `cfgkey` / `section`** on `$scfg` (e.g. `pvesm import` / migration workers). **`path()`** is overridden to pass **`$storeid`** into **`filesystem_path`** (upstream `Plugin::path` drops it). Password files: **`priv/storage/<storeid>.pw`**, then **`.nimble.pw`**, then **`priv/nimble/<storeid>.pw`**.
