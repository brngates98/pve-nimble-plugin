...
- **Worker password / import child:** **`check_config`** sets **`$opts->{storeid} = $sectionId`** (TrueNAS-style: section id is not otherwise present on the storage hash from **`storage_config`**). **`nimble_effective_storeid`** prefers the explicit `$storeid` argument, then **`scfg->{storeid}`**, then **`storage` / `storagename` / `name` / `id` / `cfgkey` / `section`**. **`nimble_api_credentials`** and **`nimble_api_call`** (token cache) use **`nimble_effective_storeid`**. **`path()`** forwards **`$storeid`** into **`filesystem_path`** (upstream **`Plugin::path`** drops it). Password files: **`priv/storage/<storeid>.pw`**, **`.nimble.pw`**, **`priv/nimble/<storeid>.pw`**.
