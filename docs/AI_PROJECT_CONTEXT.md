...
- **Worker password:** Redacted `$scfg` on some nodes is handled via **`nimble_effective_storeid`** and extra **`$scfg`** keys; password files are read from **`priv/storage/`** first, then **`priv/nimble/`**.
...
- **PVE property registry:** Do **not** declare `username` / `password` in Nimble’s **`properties()`** — RBD/CIFS already register them globally. List **`username` + `password` (+ optional `nimble_password`) in `options()`**. **`plugindata → sensitive-properties`**: `password`, `nimble_password`. Hooks run when storage is added/updated via **`pvesm` or API** (not a built-in custom-plugin GUI). **`pvesm`** uses underscores (`--auto_iscsi_discovery`), not hyphens, for multi-word options.
