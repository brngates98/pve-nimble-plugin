...
# Like TrueNAS (api_key) / Pure (token): `password` is stored in storage.cfg (pmxcfs-replicated). Hooks also mirror to /etc/pve/priv/storage/<id>.pw (and legacy paths) when you add or change the password so older tooling keeps working.
...
nimble: <storage_id>
  ...
  # iscsi_discovery_ips 192.168.221.120,192.168.222.120   # optional; extra portals in addition to GET subnets + subnets/:id
```
...
| iscsi_discovery_ips | **Optional.** Extra discovery portals (comma-separated; same form as `iscsiadm -m discovery -p …`). The plugin builds the primary list from **`GET v1/subnets`** and **`GET v1/subnets/:id`** for every subnet (authoritative **`discovery_ip`** / **`type`**), then merges this option, then rare fallbacks. On **map_volume**, those portals are merged with **`iscsiadm -m node --targetname`** `node.portal` records for **multipath**; retries and **global `node --login`** if the volume IQN still has no session (e.g. cold migration target).                                                                                                                                       |
...
- **iSCSI on the target node:** **`map_volume`** uses discovery portals from the **Nimble subnets API** (**`GET v1/subnets`** plus **`GET v1/subnets/:id`** per subnet), then runs sendtargets + per-portal login for the volume IQN, merges portals from the local open-iscsi node DB, rescans sessions, refreshes **multipath**, and periodically **rescans while waiting** for the disk. Use **`iscsi_discovery_ips`** only if you need **additional** portals beyond what the API returns.
...
3. **Safety:** The initiator group is ensured first (so the array knows this host's IQN); if that fails, discovery is skipped. Discovery and login are additive (they do not remove or overwrite existing iSCSI nodes). If the Nimble API cannot produce discovery portals from subnets, or if `iscsiadm` is missing, the plugin logs a warning and does not fail storage activation.
...
| **`undefined property 'nimble_password'` (or similar) in `pveproxy` / `pvedaemon` logs** | A storage key must be declared in the plugin’s **`properties()`**; PVE validates every `nimble:` section line. Remove any experimental `nimble_password` line from `/etc/pve/storage.cfg` and use **`password`** (standard option, stored in cfg). |
...
| **`Insecure dependency in exec` / `blockdev` / taint (`-T`)** | PVE runs custom storage plugins under **taint mode**. Device paths passed to **`exec`** must be **untainted** (validated with a regex capture). Use a current plugin that sanitizes `/dev/dm-*`, `/dev/sd*`, and multipath WWIDs before external commands. **iSCSI:** volume **`target_name`** (IQN) and discovery **portal** strings from the API/config are also laundered before **`iscsiadm`**; without that, **`run_command` fails inside `eval`** and you get **no session** with no obvious error. |
| **`No active iSCSI session` / migrate timeout on target node** | Confirm **data-network** reachability from the **target** to the Nimble **`discovery_ip`** values (from subnets in the API). `sudo iscsiadm -m session` on the **target** node for diagnosis. **`iscsi_discovery_ips`** is only for **extra** portals. Stale **`node.portal`** values may include a **`,tpgt`** suffix; the plugin normalizes those for discovery/login. **Logs:** `map_volume` runs on the **migration target**—check **`journalctl -u pvedaemon`** on **that** node (not only the source), and restart **`pvestatd`** there after plugin updates. **`Warning ::`** lines also show in the **migrate task log** (GUI Task History or under **`/var/log/pve/tasks/`**). When grepping journals, use **extended** regex (**`grep -E`**) if you combine several keywords; a plain quoted pattern with **`|`** is **not** alternation unless **`grep -E`** is used. |
| **Several Nimble ACL rows (e.g. `pve-pve001`, `pve-pve002`, …) on one volume** | **Normal** if each node uses its own auto initiator group: the plugin adds an access record for **that node’s** group when the volume is activated so migration and multi-node use work. It is **not** a failed “cleanup” from a bad migrate. **`deactivate_volume`** removes only **this node’s** group ACL. |
