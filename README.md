...
**Comparison with Pure Storage plugin:** Pure uses on-demand **host connections**: it connects the volume to the current host when the volume is activated and disconnects when deactivated, so migration works without a shared group. This plugin uses Nimble’s **initiator groups and ACLs**: it ensures the configured (or per-node) initiator group has a volume ACL on activate when needed, so the LUN is visible to iSCSI and the VM disk can appear.
