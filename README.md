...
# Like ESXi: `password` is sensitive — pvesm stores it in /etc/pve/priv/nimble/<storage_id>.pw
...
| password             | When set via GUI / `pvesm --password`, stored in **`/etc/pve/priv/nimble/<storage_id>.pw`** (not in `storage.cfg`), same idea as ESXi/PBS. You can still put `password` in `storage.cfg` if you manage the node by hand; the plugin reads that or the `.pw` file.                                                                                          |
...
| iscsi_discovery_ips | **Optional.** Comma-separated discovery portal addresses (same IPs you would pass to `iscsiadm -m discovery -p …`). Merged with addresses from **`GET v1/subnets`** (each subnet’s `discovery_ip`). Use if your array returns subnets in an `items` envelope we now unwrap, or you want to **force** extra portals (e.g. `192.168.221.120,192.168.222.120`).                                                                                                                                       |
...
**Comparison with [kolesa-team/pve-purestorage-plugin](https://github.com/kolesa-team/pve-purestorage-plugin):** Pure **`activate_volume`** calls the array API to **create a volume connection** (host ↔ LUN) *before* **`map_volume`**. **`map_volume`** then only runs SCSI rescan and waits for the device path by serial/WWN—it does **not** run `iscsiadm` discovery itself; operators pre-discover Pure portals once ([their README](https://github.com/kolesa-team/pve-purestorage-plugin#iscsi-configuration)). This Nimble plugin has no equivalent “connection” RPC: it sets **initiator group + access_control_records** (like ACL), then on the host runs **sendtargets + login** for the volume’s **`target_name`** IQN. Discovery portal IPs come from **GET subnets**, optional **`iscsi_discovery_ips`**, or—if both are empty—**IPs parsed from your existing `iscsiadm -m session` lines** so nodes that already talk to Nimble keep working without the subnets API.
