...
- **iSCSI on the target node:** **`map_volume`** runs discovery + per-portal login for the volume IQN, merges portals from the local open-iscsi node DB, rescans sessions, refreshes **multipath**, and periodically **rescans again while waiting** for the disk—set **`iscsi_discovery_ips`** if subnets/API do not list all data portals on your firmware.
...
| **`No active iSCSI session` / migrate timeout on target node** | On the **target** host: `sudo iscsiadm -m session`, discovery IPs (`iscsi_discovery_ips` if subnets omit paths), routing/firewall to **data** iSCSI ports, and `open-iscsi`. Stale **`node.portal`** values from open-iscsi can include a **`,tpgt`** suffix; current plugin code normalizes those for discovery/login. |
